...
- сохранения подробного отчёта в `ai-relevance.md` внутри директории урока;
...
* `/review lesson <lesson-path>`  
  Проверяет актуальность материала для одного урока, создаёт или обновляет файл `ai-relevance.md` в директории `<lesson-path>` и добавляет/обновляет сворачиваемый блок `ai-relevance` в корневом `readme.md` под соответствующим пунктом в `## Содержание`.

* `/review all`  
  Последовательно выполняет проверку актуальности для всех уроков, перечисленных в блоке `## Содержание` корневого `readme.md`. Для каждого урока создаёт/обновляет `ai-relevance.md` и блок `ai-relevance` в `readme.md`.
...
* `/review cource-plan`  
  Считывает файл `cource-plan-for-ai.md`, берёт из него ссылку на актуальный план курса, обращается по этому адресу и полностью перегенерирует секцию `## План курса` в `cource-plan-for-ai.md`. Используется для того, чтобы последующие запуски `/review` учитывали порядок уроков и не помечали как «не проработанные» темы, которые относятся к будущим урокам.

* `/review analyze <lesson-path>`  
  Выполняет анализ решения по уроку в директории `<lesson-path>`: находит пробелы в знаниях, указывает на слабые места и предлагает шаги по улучшению. Результат записывается в секцию «Анализ вашего решения» файла `ai-relevance.md` в этой директории.  
  Например:
  ```text
