...
<details>
<summary>Команда "/review"</summary>

Команда `/review` предназначена для:
- проверки актуальности материалов урока по внешней документации (через MCP `user-context7` / `@context7`);
- сохранения подробного отчёта в `ai-review.md` внутри директории урока;
- размещения краткого статуса проверки прямо в корневом `readme.md` в блоке `## Содержание`;
- анализа качества решения по уроку и указания пробелов в знаниях;
- генерации краткой инструкции по использованию Cursor AI в этом репозитории.

**Структура команды**:
- базовая форма: `/review <subcommand> [options] <lesson-path?>`;
- `<lesson-path>` — относительный путь к директории урока (например, `20-ts/40-classes/110-this-and-typed-context`);
- опции (ключи) передаются после подкоманды (например, `--skip`).

**Поддерживаемые подкоманды**:

* `/review lesson <lesson-path>`  
  Проверяет актуальность материала для одного урока, создаёт или обновляет файл `ai-review.md` в директории `<lesson-path>` и добавляет/обновляет сворачиваемый блок `ai-relevance` в корневом `readme.md` под соответствующим пунктом в `## Содержание`.

* `/review all`  
  Последовательно выполняет проверку актуальности для всех уроков, перечисленных в блоке `## Содержание` корневого `readme.md`. Для каждого урока создаёт/обновляет `ai-review.md` и блок `ai-relevance` в `readme.md`.

* `/review all --skip <days>`  
  То же, что `/review all`, но пропускает уроки, у которых дата последней проверки (`Дата проверки` в `ai-relevance.md`) не старше указанного количества дней `<days>`.

* `/review cource-plan-for-ai`  
  Считывает файл `cource-plan-for-ai.md`, берёт `plan-url` из секции `## "ai-options"` (ini-блок), обращается по этому адресу и полностью перегенерирует секцию `## "cource-plan"` в `cource-plan-for-ai.md`. Используется для того, чтобы последующие запуски `/review` учитывали порядок уроков и не помечали как «не проработанные» темы, которые относятся к будущим урокам.

* `/review analyze <lesson-path>`  
  Выполняет анализ решения по уроку в директории `<lesson-path>`: находит пробелы в знаниях, указывает на слабые места и предлагает шаги по улучшению. Результат записывается в секцию «Анализ вашего решения» файла `ai-review.md` в этой директории.  
  Например:
  ```text
  /review analyze "20-ts/40-classes/110-this-and-typed-context"
  ```
  При необходимости можно помечать участки кода маркерами, связанными с конкретными пунктами анализа в `ai-review.md`.  
  Формат маркера:
  ```text
  @ai-review|Анализ вашего решения/Дата: "<YYYY-MM-DD HH:MM>"/<подраздел>/<номер>|<free-text>
  ```
  где `<подраздел>` — один из блоков внутри анализа (например, `Пробелы в знаниях`, `Замечания к реализации`, `Рекомендованные шаги`), а `<номер>` — номер пункта в соответствующем списке.

  Примеры:
  ```ts
  // @ai-review|Анализ вашего решения/Дата: "2026-03-16 10:31"/Рекомендованные шаги/2|Implement birthday type
  ```
  ```bash
  # @ai-review|Анализ вашего решения/Дата: "2026-03-16 10:31"/Рекомендованные шаги/2|CLI script handling birthday
  ```

* `/review guide`  
  Создаёт или обновляет файл `cursor-ai-usage.md` в корне репозитория с инструкцией по использованию Cursor AI и команд (`/van`, `/plan`, `/build`, `/review` и др.), и гарантирует наличие ссылки на этот файл в секции `### Помощник AI` корневого `readme.md`.
